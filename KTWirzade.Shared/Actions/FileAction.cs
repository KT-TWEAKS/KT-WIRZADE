    ...
    public class FileAction : Tasks.TaskAction, ITaskAction
    {
                ...
                if (File.Exists(file))
                {
                    ...
                    while (processes.Any() && delay <= 800)
                    {
                        ...
                        foreach (var process in processes)
                        {
                            try
                            {
                                if (process.ProcessName.Equals("KTWirzade.CLI"))
                                {
                                    output.WriteLineSafe("Info", "Skipping KTWirzade.CLI...");
                                    continue;
                                }
        ...
        public async Task<bool> RunTask(Output.OutputWriter output)
        {
            ...
            if (realPath.Contains("*"))
            {
                ...
                if (lastToken < 0)
                {
                    output.WriteLineSafe("Warning", $"Invalid file path '{realPath}': no directory separator found.");
                    InProgress = false;
                    return true;
                }
                var parentPath = realPath.Remove(lastToken).TrimEnd('\\');
