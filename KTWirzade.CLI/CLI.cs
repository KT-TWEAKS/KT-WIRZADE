...
namespace KTWirzade.CLI
{
    public class CLI
    {
        ...
        public static void ExtractResourceFolder(string resource, string dir, bool overwrite = false)
        {
            ...
            var resources = assembly.GetManifestResourceNames().Where(res => res.StartsWith($"KTWirzade.CLI.Properties.{resource}."));
            ...
            foreach (var obj in resources)
            {
                using (UnmanagedMemoryStream stream = (UnmanagedMemoryStream)assembly.GetManifestResourceStream(obj))
                {
                    ...
                    var file = dir + $"\\{obj.Substring($"KTWirzade.CLI.Properties.{resource}.".Length).Replace("---", "\\")}";
                    ...
                    using (FileStream fsDlst = new FileStream(file, FileMode.CreateNew, FileAccess.Write))
                    {
                        ...
                        {
                            int bytesToRead = (int)Math.Min(MB, stream.Length - offset);
